...
import React, { Component, ComponentType, ReactNode, ChildContextProvider, ClassAttributes, ValidationMap } from 'react';
import PropTypes from 'prop-types';
import { toJS, extendObservable, action, runInAction, observable } from 'mobx';
import AsyncValidator, { Descriptor, DescriptorObject, DescriptorFnObject, ValidatorCb, ValidationError } from 'async-validator';
...
export function createForm<S extends t.FormStore = t.FormStore, P extends t.FormHOCProps<S> = t.FormHOCProps<S>>(
	opts: t.CreateFormOption<S> = {},
): t.FormDecorator<S, P> {
	const { store: gStore, prefix = '', defaultItemProps = {}, displayDefaultLabel = true } = opts;

	return (WrappedComponent: ComponentType<P & ClassAttributes<any>>) =>
		class WrapForm extends Component<Omit<P, keyof t.FormHOCProps> & t.FormHOCPropsExtra<S>>
			...
			public validateFields = (callback?: (fields: any) => any): Promise<any> => {
				const needValidateName: string[] = [];
				const descriptor: Descriptor = Object.keys(this.fieldOptions).reduce((o, name) => {
			...
			public getFieldProps = (name: string, custom: t.FieldOption = {}) => {
				...
				const { trigger, validateTrigger, valuePropName, parseValue, appendProps, initialValue } = fieldOption;
			...
			private setField = (path: string, value: any) => {
				const store = this.getStore();
				return runInAction('setField', () => set(store, prefix ? `${prefix}.${path}` : path, value));
			};
			...
			private createHandler = ({ name, onChange, rules, valueFromEvent }: t.FieldOption, needValidate = false) => {
				return (...params: any[]) => {
			...
			private createValidateHandler = ({ name, rules, valueFromEvent }: t.FieldOption) => {
				return (...params: any[]) => {
					const value = valueFromEvent(...params);
					this.validateField(name, value, rules);
				};
