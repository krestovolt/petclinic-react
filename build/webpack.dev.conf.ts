		...
		devServer: {
			...
			historyApiFallback: {
				rewrites: [
					{
						from: /.*/,
						to: path.posix.join(config.common.paths.assetPublicPath, 'index.html'),
					},
				],
			},
			hot: true,
			...
			overlay: config.dev.server.errorOverlay ? { warnings: false, errors: true } : false,
...
export default new Promise((res, rej) => {
	...
	portfinder.getPort((err, port) => {
		if (err) {
			...
			(devWebpackConfig as any).devServer.port = port;
